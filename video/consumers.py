...
from modely.models import Club,User_Club 
from auth_api.models import Users
from django.contrib.auth.models import AnonymousUser
from asgiref.sync import sync_to_async

import asyncio

#podla tutorialu https://www.youtube.com/watch?v=MBOlZMLaQ8g

class Consumer(AsyncWebsocketConsumer):
    
    async def connect(self):
        self.room_group_name = "chat_" + str(self.scope["url_route"]["kwargs"]["clubid"])
        ell = Club.objects.filter(id=self.scope["url_route"]["kwargs"]["clubid"])
        exists = await sync_to_async(ell.exists)()
        if not exists:
            await self.close()
            return
